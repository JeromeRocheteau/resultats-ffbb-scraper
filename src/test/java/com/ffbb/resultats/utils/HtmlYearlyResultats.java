...
public class HtmlYearlyResultats extends ResultatsExtraction {
	...
	public HtmlYearlyResultats() throws Exception {
		super();
		...
		filtreChampionnat = new ChampionnatFiltre().catégories(Catégorie.U9);
		filtreNort = new MultipleFiltres().filtres(filtreChampionnat);
		/*
		filtreSaffré = new ChampionnatFiltre()
		...
		// organisations.put("223c", filtreSaffré);
	}
		...
		for (Engagement engagement : engagements) {
			...
			if (filtre.match(compétition)) {
				...
				if (championnat.getCatégorie() == Catégorie.U9) {
					...
					Classement classement = this.getClassement(organisation, championnat, rencontres);
	...
	private Classement getClassement(Organisation organisation, Championnat championnat, List<Rencontre> rencontres) {
		Équipe équipe = null;
		int rang = 0;
		int victoires = 0;
		int défaites = 0;
		...
		for (Rencontre rencontre : rencontres) {
			Équipe domicile = rencontre.getDomicile();
			Équipe visiteur = rencontre.getVisiteur();
			if (rencontre.getRésultat() == null) {
				continue;
			}
			int domicileScore = rencontre.getRésultat().getDomicile();
			int visiteurScore = rencontre.getRésultat().getVisiteur();
			if (domicile.getOrganisation().getCode() == organisation.getCode()) {
				if (équipe == null) {
					équipe = domicile;
				}
				pour += domicileScore;
				contre += visiteurScore;
				if (domicileScore > visiteurScore) {
					victoires++;
				} else if (domicileScore < visiteurScore) {
					défaites++;
				} else {
					this.doWarn("match nul");
				}
			} else if (visiteur.getOrganisation().getCode() == organisation.getCode()) {
				if (équipe == null) {
					équipe = visiteur;
				}
				pour += visiteurScore;
				contre += domicileScore;
				if (domicileScore > visiteurScore) {
					victoires++;
				} else if (domicileScore < visiteurScore) {
					défaites++;
	...
	private void doWrite() throws Exception {
		...
		for (Championnat championnat : championnats) {
			...
			this.doBody(championnat, classement, html);
		}
		File file = new File("/home/jerome/Bureau/naclt-all-resultats.html");
	...
	private void doBody(Championnat championnat, Classement classement, StringBuilder html) {
		String link = this.doLink(championnat);
		html.append("<p>");
		html.append("<strong>");
	...
	private void doPhase(Championnat championnat, StringBuilder html) {
		Integer phase = championnat.getPhase();
		html.append(phase);
		if (phase.intValue() == 1) {
	...
	private String doLink(Championnat championnat) {
		String id = championnat.getCode();
		Long d = championnat.getId();
		Long r = 0L; // FIXME championnat.getParamètres().getR();
		return "http://resultats.ffbb.com/championnat/" + id + ".html?r=" + r.toString() + "&d=" + d.toString();
	}
